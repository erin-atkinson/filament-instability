...
q_vol_func(i, j, k, grid, q) = @inbounds q[i, j, k] <= 0 ? grid.Δxᵃᵃᶜ * grid.Δyᵃᶜᵃ * grid.Δzᵃᵃᶜ[k] : 0
q_vol_op = KernelFunctionOperation{Center, Nothing, Center}(q_vol_func, grid, q)
